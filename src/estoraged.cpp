                              ...
                              std::vector<uint8_t> password)
{
    ...
    int retval =
        cryptIface->cryptFormat(cd, CRYPT_LUKS2, "aes", "xts-plain64", NULL,
                                ...
                                std::vector<uint8_t> password)
{
    ...
    int retval = cryptIface->cryptLoad(cd, CRYPT_LUKS2, NULL);
    if (retval < 0)
    {
...
void eStoraged::deactivateLuksDev()
{
    ...
    int retval = cryptIface->cryptDeactivate(NULL, containerName.c_str());
    if (retval < 0)
    {
