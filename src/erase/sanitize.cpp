...
void Sanitize::doSanitize()
{
    ...
    // save the size of drive for use later
    uint64_t driveSize = findSizeOfBlockDevice();
    ...
    try
    {
        ...
        (void)ioctlWrapper->doIoctl(fd, MMC_IOC_CMD, idata);
        ...
        (void)ioctlWrapper->doIoctlMulti(fd, MMC_IOC_MULTI_CMD, erase_cmd);
    }
    catch (...)
    {
...
int IOCTLWrapperImpl::doIoctlMulti(ManagedFd& fd, unsigned long request,
