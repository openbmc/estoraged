...
void Pattern::writePattern(const uint64_t driveSize, ManagedFd& fd)
{
    ...
    std::array<std::byte, BLOCK_SIZE> randArr;
    while (currentIndex < driveSize)
    {
        // generate a 4k block of prng
        std::array<uint32_t, BLOCK_SIZE / sizeof(uint32_t)>* randArrFill =
            reinterpret_cast<
                std::array<uint32_t, BLOCK_SIZE / sizeof(uint32_t)>*>(&randArr);
        ...
        size_t writeSize = currentIndex + BLOCK_SIZE < driveSize
                               ? BLOCK_SIZE
...
void Pattern::verifyPattern(const uint64_t driveSize, ManagedFd& fd)
{
    ...
    std::array<std::byte, BLOCK_SIZE> randArr;
    std::array<std::byte, BLOCK_SIZE> readArr;
    while (currentIndex < driveSize)
    {
        ...
        try
        {
            std::array<uint32_t, BLOCK_SIZE / sizeof(uint32_t)>* randArrFill =
                reinterpret_cast<
                    std::array<uint32_t, BLOCK_SIZE / sizeof(uint32_t)>*>(
                    &randArr);
            for (uint32_t i = 0; i < BLOCK_SIZE / sizeof(uint32_t); i++)
            {
                (*randArrFill)[i] = generator();
