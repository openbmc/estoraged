...
TEST_F(EStoragedTest, FormatPass)
{
    ...
    EXPECT_CALL(*mockFsIface, runMkfs(StrEq(esObject->getCryptDevicePath()))).WillOnce(Return(0));
...
TEST_F(EStoragedTest, CreateFilesystemFail)
{
    ...
    EXPECT_CALL(*mockFsIface, runMkfs(StrEq(esObject->getCryptDevicePath()))).WillOnce(Return(-1));
