...
class MockeStoraged : public estoraged::eStoraged
{
  public:
    ...
    MOCK_METHOD(int, runMkfs, (), (override));
...
TEST_F(eStoragedTest, FormatPass)
{
    ...
    EXPECT_CALL(*esObject, runMkfs()).WillOnce(Return(0));

    EXPECT_CALL(*esObject, createDirectory(path(esObject->getMountPoint())))
        .WillOnce(Return(true));

    EXPECT_CALL(*esObject, doMount(ContainsRegex("/dev/mapper/"),
                                   StrEq(esObject->getMountPoint()), _, _, _))
        .WillOnce(Return(0));

    EXPECT_CALL(*esObject, doUnmount(StrEq(esObject->getMountPoint())))
        .WillOnce(Return(0));

    EXPECT_CALL(*esObject, removeDirectory(path(esObject->getMountPoint())))
        .WillOnce(Return(true));
...
TEST_F(eStoragedTest, CreateFilesystemFail)
{
    ...
    EXPECT_CALL(*esObject, runMkfs()).WillOnce(Return(-1));
