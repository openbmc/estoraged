...
int main(int argc, char** argv)
{
    ...
    if (containerBlockDev.empty()) {
        containerBlockDev = "luks-";
        containerBlockDev.append(deviceName);
    }

    /* DBus path location to place the object. */
    std::string path = "/xyz/openbmc_project/storage/";
    ...
    estoraged::eStoraged es_object{b, path.c_str(),
      physicalBlockDev, containerBlockDev};
