...
{
    std::cerr << "in do sanitize" << std::endl;
    try
    {
    ...
    std::cerr << "do with do santize" << std::endl;
}
...
void Sanitize::emmcErase(uint64_t driveSize, std::array<std::byte, 512> extCsd)
{
    ...
    std::cerr << "starting eMMC Erase" << std::endl;
    ...
    else
    {
        ...
        eraseSize = static_cast<uint64_t>(
            0x80000 * static_cast<uint32_t>(extCsd[extCsdHcEraseGrpSize]));
        std::cerr << static_cast<uint32_t>(extCsd[extCsdHcEraseGrpSize])
                  << std::endl;
    }
    std::cerr << eraseSize << std::endl;
    ...
    std::cerr << (driveSize / eraseSize) << std::endl;
    ...
    std::cerr << "done eMMC Erase" << std::endl;
}
