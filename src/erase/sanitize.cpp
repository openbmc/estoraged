...
void Sanitize::emmcErase(uint64_t driveSize, std::span<std::byte>& extCsd)
{
    ...
    // The tiwddle was added as a way to keep our code spec compliant and work
    // with a device that is not
    uint64_t eraseSize;
    if ((extCsd[EXT_CSD_ERASE_GROUP_DEF] & std::byte{1}) == std::byte{1} ||
        twiddleEXT_CSD_ERASE_GROUP_DEF)
    {
    ...
    if (twiddleEXT_CSD_ERASE_GROUP_DEF)
    {
