...
int sanitize::wrapperIOCTL(int fd, unsigned long request,
                           struct mmc_ioc_cmd data)
{
    return ioctl(fd, request, data);
}

sanitize::sanitize(std::string inDevPath, unsigned int delaySec) :
    Erase(inDevPath)
{

    std::cout << "in sanitize constructor" << std::endl;
    ManagedFd fd(open(devPath.c_str(), O_RDONLY, O_NDELAY));
    ...
    // make the ioctl
    std::cout << "calling ioctl " << std::endl;
    int ret = this->wrapperIOCTL(fd.get(), MMC_IOC_CMD, idata);
