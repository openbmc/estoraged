    ...
    size_t retries)
{
    auto getter = std::make_shared<estoraged::GetStorageConfiguration>(
        dbusConnection,
        [&io, &objectServer, &storageObjects, &dbusConnection](
            ...
            {
                std::string path = storage.first.str;
                ...
                estoraged::StorageData data = storage.second;

                /* Look for the device file. */
                std::filesystem::path blockDevDir{"/sys/block"};
                ...
                uint64_t size =
                    estoraged::util::findSizeOfBlockDevice(deviceFile.string());
                ...
                storageObjects[path] = std::make_unique<estoraged::EStoraged>(
                    objectServer, deviceFile.string(), luksName, size,
                    lifeleft);
    ...
    getter->getConfiguration(retries);
}
...
int main(void)
{
    try
    {
        ...
        boost::asio::post(io, [&]() {
            createStorageObjects(io, server, storageObjects, conn, 0);
        });
